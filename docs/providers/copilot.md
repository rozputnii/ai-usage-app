...
[Authorizing OAuth apps](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps) (inspected 2026-09-16) adds constraints that are shared with every OpenCode user because the client is reused. GitHub issues at most ten tokens per user/application/scope and revokes an existing one beyond that, so AI Usage connections and OpenCode logins can revoke each other's tokens. Creating more than ten tokens per hour triggers a re-authorization prompt. Code submissions are limited to 50 per hour per application. A user who previously granted OpenCode broader scopes can receive a token with those scopes. AI Usage treats every issued token as consumed: token polls and the identity binding ignore caller cancellation, within the 15-second request deadline.
