...
The console uses internal `AddCodexIntegration` through explicit friend-assembly access to obtain the shared typed clients with redirects, cookies and HTTP factory logging disabled. Product consumers use `AddCodexProductSession` and the credential-free session operations; clients and session constructors are internal, so ordinary consumers cannot supply an arbitrary HTTP pipeline. The console contains no separate provider HTTP implementation or WinUI dependency; its Codex login remains in memory.
