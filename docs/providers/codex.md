...
backlog: AIU-003
---
# codex — implementation evidence assignment

Confirm the current ChatGPT-account Codex quota source, including Spark or other additional limits only when present in real responses. Establish browser/device flow availability, exact account/workspace claims and refresh-token coexistence when the CLI and app both run. API billing usage is not Codex subscription quota.
