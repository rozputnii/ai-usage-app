...
No real fixture has been captured and no Claude parser test has run. Future fixtures must be explicitly synthetic or captured with owner consent and sanitized. Use synthetic account/organization IDs, never real tokens, email or account payloads. Required cases include valid/missing/conflicting identity, legacy and generic shared limits, multiple scoped rows including `is_active: false`, unknown kinds and duplicate names, null/invalid/exhausted percentages, ISO offsets and malformed resets, enabled/disabled/unknown extra spend, currency/exponent mismatch, malformed JSON and secret-bearing error payloads.
