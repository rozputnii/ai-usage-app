...
Publication amendment, 2026-09-15: after successful live verification and local-main integration, the owner explicitly requested "commit and push, mark as done if all is fine". This authorizes publishing the completed task branch and the verified local main to the configured origin, superseding the earlier local-only publication constraint. It does not authorize a release or establish provider permission.
